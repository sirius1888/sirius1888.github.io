...
| ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------- |
| [App](../src/App.tsx)                                                    | Без props; собирает навигацию, шесть разделов и подвал.                                                                                     | Подключает `usePreferences`; состояния демо здесь нет.                             |
...
| [SiteFooter](../src/components/layout/SiteFooter.tsx)                    | `lang`; подпись и ссылка возврата наверх после прокрутки первого экрана.                                                                    | Локальное состояние видимости ссылки; подписки на scroll/resize очищаются при размонтировании. |
...
| [ProfileHero](../src/features/profile/ProfileHero.tsx)                   | `lang`; представление разработчика и Team Lead, основной контакт, резюме, фотография и ссылки на продукты.                                                                             | Нет.                                                                               |
| [SkillsPanel](../src/features/profile/SkillsPanel.tsx)                   | `lang`; самостоятельный раздел после кейсов: все категории навыков и показатели опыта.                                                                                          | Нет; читает `skillGroups`.                                                         |
...
| [MuseSection](../src/features/advertising/MuseSection.tsx)               | `lang`; личные задачи, результат для команды, рекламное демо и раскрываемая техническая схема.                                                                                          | `useAdConfiguration` связывает конфигурацию двух представлений.                    |
...
| [AdShowcaseControls](../src/features/advertising/AdShowcaseControls.tsx) | Только `consent` из конфигурации, `app`, `format`, `onAppChange`, `onFormatChange`, `onConsentChange`, `lang`; управляемые элементы выбора.             | Нет.                                                                               |
...
`useScrollReveal` возвращает ref для основного содержимого и один раз анимирует помеченные `data-reveal` блоки при прокрутке. При отсутствии IntersectionObserver содержимое остаётся видимым; подписки очищаются при размонтировании. Декоративные эффекты описаны в `src/motion.css` и отключаются через `prefers-reduced-motion`.
